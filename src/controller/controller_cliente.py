...
class Controller_Cliente:
    ...
    def inserir_cliente(self) -> Cliente:
        ...
        if self.verifica_existencia_cliente(oracle, cpf):
            ...
            RG = input("RG (Novo): ")
            # Solicita ao usuario a nova data de nascimento
            Data_nascimento = pd.to_datetime(input("Data de Nascimento, digite em formato de M-D-A (Novo): "))
            # Solicita ao usuario o novo CNH
            CNH = input("CNH (Novo): ")
            # Solicita ao usuario o novo endereco
            endereco = input("endereco (Novo): ")
            # Insere e persiste o novo cliente
            oracle.write(f"insert into clientes values ('{cpf}', '{rg}', '{data_nascimento}', '{cnh}', '{nome}', '{endereco}')")
            # Recupera os dados do novo cliente criado transformando em um DataFrame
            df_cliente = oracle.sqlToDataFrame(f"select cpf, rg, data_nascimento, cnh, nome, endereco from clientes where cpf = '{cpf}'")
            # Cria um novo objeto Cliente
            novo_cliente = Cliente(df_cliente.cpf.values[0], df_cliente.rg.values[0], df_cliente.data_nascimento.values[0], df_cliente.cnh.values[0], df_cliente.nome.values[0], df_cliente.endereco.values[0])
    ...
    def atualizar_cliente(self) -> Cliente:
        ...
        if not self.verifica_existencia_cliente(oracle, cpf):
            ...
            oracle.write(f"update clientes set endereco = '{novo_endereco}' where cpf = {cpf}")
            # Recupera os dados do novo cliente criado transformando em um DataFrame
            df_cliente = oracle.sqlToDataFrame(f"select cpf, rg, data_nascimento, cnh, nome, endereco from clientes where cpf = {cpf}")
            # Cria um novo objeto cliente
            cliente_atualizado = Cliente(df_cliente.cpf.values[0], df_cliente.rg.values[0], df_cliente.data_nascimento.values[0], df_cliente.cnh.values[0], df_cliente.nome.values[0], df_cliente.endereco.values[0])
            # Exibe os atributos do novo cliente
    ...
    def excluir_cliente(self):
        ...
        if not self.verifica_existencia_cliente(oracle, cpf):            
            ...
            cliente_excluido = Cliente(df_cliente.cpf.values[0], df_cliente.rg.values[0], df_cliente.data_nascimento.values[0], df_cliente.cnh.values[0], df_cliente.nome.values[0], df_cliente.endereco.values[0])
